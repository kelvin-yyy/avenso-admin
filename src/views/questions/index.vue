<template>
  <div class="dashboard-container">
    ...
    <el-table :data="tableData" v-loading="loading" border style="width: 100%">
      <el-table-column prop="questionsId" label="公司id">
      </el-table-column>
    ...
    <el-dialog :title="state==1?'新增':'修改'" :visible.sync="newdialog"   width="30%" :before-close="newClose">
      <el-form ref="newform" :model="newform" label-width="80px">
        <el-form-item label="分类">
          <el-select class="selectClass" v-model="newform.classification" clearable placeholder="请选择问题分类">
            <el-option v-for="item in questionsType" :key="item.id" :label="item.name" :value="item.id">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="问题">
          <el-input v-model="newform.content" type="textarea" :rows="6"></el-input>
        </el-form-item>
        <el-form-item label="公司">
           <el-select v-model="newform.questionsId" placeholder="请选择">
              <el-option
  ...
  export default {
    ...
    data() {
      return {
        ...
        questionsType:[
          {name:"fundingRounds",id:"fundingRounds"},
     ...
     handleClick(row){
            ...
            this.$confirm('此操作将删除该about, 是否继续?', '提示', {
              ...
              type: 'warning'
                ...
                deletequestions({id:row.id}).then((res)=>{
                  this.list()
                  this.$message({
                    type: 'success',
                    message: '删除成功'
      ...
      establishChecked(){
        ...
        companyquestions(this.newform).then((res)=>{
          this.newdialog=false
          this.list()
        })
      },
      updateChecked(){
        updatequestions(this.newform).then((res)=>{
          this.newdialog=false
          this.resetFields()
          this.list()
        })
      ...
      list(){
        newquestions(this.form).then((res)=>{
