<template>
  <div class="dashboard-container">
    ...
    <el-table :data="tableData" v-loading="loading" border style="width: 100%">
      <el-table-column prop="employeeId" label="公司id">
      </el-table-column>
      <el-table-column prop="name" label="名子">
      </el-table-column>
    ...
    <el-dialog :title="state==1?'新增':'修改'" :visible.sync="newdialog" width="30%" :before-close="newClose">
      <el-form ref="newform" :model="newform" label-width="80px">
        ...
        <el-form-item label="公司">
          <el-select v-model="newform.employeeId" placeholder="请选择">
  ...
  export default {
    ...
    methods: {
      ...
      updateClick(row) {
        ...
        this.newform = row
        this.state = 2
        this.fileList.push({url:row.Avatar})

      },
      handleClick(row) {
        // var that
        this.$confirm('此操作将删除该employee, 是否继续?', '提示', {
      ...
      establishChecked() {
        this.fileList.forEach((res)=>{
          this.newform.Avatar=res.url
        })
        companyemployee(this.newform).then((res) => {
      ...
      updateChecked() {
        this.newform.Avatar=[]
        this.fileList.forEach((res)=>{
          this.newform.Avatar.push(res.url)
        })
        updateemployee(this.newform).then((res) => {
          this.newdialog = false
          this.resetFields()
          this.list()
        })
      ...
      list() {
        newemployee(this.form).then((res) => {
