<template>
  <div class="dashboard-container">
    ...
    <el-table :data="tableData" v-loading="loading" border style="width: 100%">
      ...
      <el-table-column prop="num" label="组织数量">
      </el-table-column>
      <el-table-column prop="Funding" label="资金来源">
      </el-table-column>
      <el-table-column prop="numberOfinvestors" label="投资人数">
      </el-table-column>
    ...
    <el-dialog :title="state==1?'新增':'修改'" :visible.sync="newdialog" width="30%" :before-close="newClose">
      <el-form ref="newform" :model="newform" label-width="80px">
        ...
        <el-form-item label="上传图片">
          <el-upload action="#" :http-request="handleFileUpload" :file-list="fileList" list-type="picture-card"
            :on-preview="handlePictureCardPreview" :limit="4" :on-remove="handleRemove">
            <i class="el-icon-plus"></i>
          </el-upload>
        </el-form-item>
        <el-form-item label="公司">
          <el-select v-model="newform.companyId" placeholder="请选择">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleClick(row) {
        // var that
        this.$confirm('此操作将删除该featurin, 是否继续?', '提示', {
      ...
      establishChecked() {
        this.fileList.forEach((res)=>{
          this.newform.img.push(res.url)
        })
        companyfeaturin(this.newform).then((res) => {
      ...
      updateChecked() {
        ...
        updatefeaturin(this.newform).then((res) => {
          this.newdialog = false
          this.resetFields()
          this.list()
        })
      ...
      list() {
        newfeaturin(this.form).then((res) => {
